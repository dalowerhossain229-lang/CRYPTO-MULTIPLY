...
app.get('/api/crypto-balance', async (req, res) => {
    ...
    try {
        ...
        console.log("Crypto Balance Stream Reconnected.");
        return res.json({ success: false, balance: 0 }); 
    }
});

// 🛫 ২. ক্রিপ্টো মাল্টিপ্লাই কোর ট্রানজেকশন ট্রেড রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/crypto-deal', async (req, res) => {
    const { userId, amount, wallet, prediction } = req.body; // prediction: 'GREEN' (UP) অথবা 'RED' (DOWN)
    const reqAmount = parseFloat(amount) || 50;
    const userPrediction = String(prediction || "GREEN").toUpperCase(); 
    const finalGameName = "cryptomultiply"; // 🎯 লবির কি-শর্টকোড টাইট লক
    const targetWallet = wallet || "main";

    if (reqAmount < 1 || reqAmount > 20000 || !["GREEN", "RED"].includes(userPrediction)) {
        return res.json({ success: false, message: "🚨 Invalid Bet Parameter! Select GREEN or RED." });
    }
    ...
    try {
        ...
        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP এবং মার্কেট ট্রেন্ড জেনারেটর লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 150) {
            loopSafety++;
            
            // র্যান্ডম বুলিশ/বেয়ারিশ ট্রেন্ড সিলেকশন (GREEN অথবা RED)
            finalOutcomeTrend = (Math.random() > 0.5) ? "GREEN" : "RED";

            if (userPrediction === finalOutcomeTrend) {
                finalStatus = "win";
                winMultiplier = 1.95; // 🎯 আপনার স্ক্রিনশটের ওরিজিনাল ওডস x1.95 গুণ প্রফিট ক্রেডিট ওস্তাদ!
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (balResponse.data && balResponse.data.crypto_target) {
                let target = String(balResponse.data.crypto_target).toUpperCase();
                if (target === "FORCE_LOSE" && finalStatus === "win") isLoopActive = false;
                if (target === userPrediction && finalStatus === "win") isLoopActive = false;
            } else {
                if (finalStatus === "win") {
                    // ক্রিপ্টো সুপ্রিম আরটিপি স্বাভাবিক ক্যাসিনো ট্র্যাকে ৪৩% এ ব্যালেন্সড লক ভাই ভাই!
        ...
        if (response.data && response.data.status === "ok") {
            ...
            return res.json({
                ...
                gameData: { outcomeTrend: finalOutcomeTrend, status: finalStatus, winAmount }
            });
        } else {
...
io.on('connection', (socket) => { console.log("Player connected to Crypto Live Trading Engine!"); });

// ⚡ কাস্টম ক্রিপ্টো মাল্টিপ্লাই নোড সার্ভার পোর্ট গেটওয়ে লাইভ অন ফায়ার
const PORT = process.env.PORT || 26000; // 🎯 ক্রিপ্টো মাল্টিপ্লাইয়ের জন্য ডেডিকেটেড পোর্ট ৩৪০০০ লক ভাই ভাই
server.listen(PORT, () => { console.log(`🎡 Crypto Multiply Engine Running on port ${PORT}`); });
